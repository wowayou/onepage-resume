...
`output_basename`，一边改一边告诉你将生成 `X.pdf / .png / .html` 到哪个目录。
**撞名时不会闷头覆盖**，而是让你二选一：自动改名为 `X-2.pdf`，或者覆盖已有的那份
（覆盖前照样先留历史版本——那是内容文件的机制，生成物没有）。

生成完，状态栏下面列出三个文件，每个都带四个动作：
...
| 「另存到…」按钮是灰的 | 浏览器没有 File System Access API（Firefox、Safari 都没有） | 用「下载」；或者换 Chrome / Edge 打开这个页面 |
