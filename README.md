...
"标题压住上一段文字"这种事。默认就是一页，超页时程序**报错拒绝生成**，不会偷
偷给你多一页；确实需要多页时，把页数上限（`max_pages`）调到 2–5 页再生成（见下文）。
...
| **历史快照** `content.toml` 旁边的 `.history/` | 内容目录下 | ❌ 被 `.gitignore` 挡住 |
...
1. `.gitignore` 里是 `content*.toml` + `!content.example.toml`——**除了示例，任何
   ...
   `.history/` 另有一条——每次覆盖前留下的快照和主文件一模一样，装的同样是真实内容。
   `*.toml.bak` 也还留着，挡住从前那些老备份。
...
- **页数 1 ✓ / 2 ✗**——超过页数上限当场变红，不用等点了"生成 PDF"才发现。页数
  上限默认是 1；写了 `max_pages` 就按那个数字判。
- **空白 N 处**——点一下列出还差哪些空，再点其中一条就跳到那个输入框；鼠标停上去
  也能看到同一份清单。未完成校验、有空白或超过页数上限时，"生成 PDF"都禁用，
  避免把旧预览当成当前内容的校验结果。
- 右边是**文件名 + 状态徽标**：未命名 / 未保存 / 已保存 14:20 / 只读（附原因）。
- 数组块（联系方式、技能、经历、项目）可以加一条、删一条、上下移。
- 正文板块（技能 / 经历 / 项目 / 教育）的标题旁有一排小按钮：上移 / 下移 / 移除。
  **移除内建块只是不再渲染，数据仍留在文件里**，表单末尾的"加板块"里可以把它加回来。
  表单末尾还能**新建自定义板块**：自己定栏目名和若干字段（单行 / 多行 / 一行多项），
  用一套通用版面渲染（第一个字段当标题）——版面不如内建块那么精调，换来的是字段完全自定义。
- 换 `theme.en.toml` 立刻重渲，中英文版式可以来回比。

点**生成 PDF** 先弹一个框，把"会写到哪"说清楚：文件名预填 `[document]` 里的
`output_basename`，一边改一边告诉你这个名字会落到哪。**存到哪有两条路，在生成之前选**：

| 存到哪 | 结果 |
|---|---|
| **存到 `build/`**（默认） | `X.pdf / .png / .html` 三份落在生成物目录，可回看、可重下、可"在文件夹中显示" |
| **只存到我选的位置** | 弹系统保存对话框，PDF 直接写到你挑的地方，**生成物目录里不留副本** |

第二条路里，服务端渲完就把 PDF 字节直接回给浏览器，磁盘上不落任何一份——
所以也没有 PNG、没有 HTML、没有下面那张结果表，取走的就是你选的那一个文件。
想要三件套或者想以后再回看，用默认那条。选择保存位置靠浏览器的 File System Access
API，Firefox / Safari 没有，那里只剩默认这条路（对话框里会直接说明）。

**撞名时不会闷头覆盖**，而是让你二选一：自动改名为 `X-2.pdf`，或者覆盖已有的那份
（覆盖前照样先留历史版本——那是内容文件的机制，生成物没有）。只存到自选位置时不问撞名：
那个目录里覆盖谁，由系统保存对话框自己负责。

生成到 `build/` 之后，状态栏下面列出三个文件，每个都带四个动作：

| 动作 | 说明 |
|---|---|
| **查看** | 新标签页里用浏览器自带的阅读器打开（只有 PDF 和 PNG 有） |
| **下载** | 存到浏览器的默认下载位置 |
| **另存到…** | 弹出系统保存对话框，自己挑位置和文件名 |
| **在文件夹中显示** | 让文件管理器打开那个目录并选中文件 |

**另存到…** 用的是浏览器的 File System Access API，只有 Chromium 系（Chrome / Edge）
有，所以那个按钮在 Firefox、Safari 里是灰的——用**下载**代替即可。**在文件夹中显示**
在 WSL 里走 Windows 的 `explorer.exe`，Linux 桌面走 `xdg-open`，都没有时会告诉你
文件的完整路径，可以复制走。
...
| 按钮 | 做什么 |
|---|---|
| **文件** | 下拉里选一份就打开它（示例和定制版也能打开看版面，只是存不回去） |
| **新建** | 从一份空表单开始，状态是"未命名" |
| **保存** / Ctrl-S | 写回当前文件；还没起名字时会转成"另存为" |
| **另存为…** | 一边打字一边告诉你这个名字会存成什么、在不在；已存在时主按钮变成"覆盖" |
| **历史版本…** | 列出这个文件的历次快照（时间、大小），可以把某一版读回表单 |
| **?** | 五行说明，外加当前的内容目录与生成物目录 |

改动只在浏览器里，点**保存**才写进文件。有未保存的改动时切换文件、点新建、关标签页
都会先问一句。停手一秒后，当前内容还会存一份**草稿**在浏览器本地，下次打开同一份
文件时问你要不要接着改——草稿只在浏览器里，换台机器就没有。

**每次覆盖之前，上一版都会自动留在内容目录的 `.history/<文件名>/` 里**（最近 30 份），
保存成功后提示会写明这一次留了没有。写盘仍是先写同目录临时文件再原子替换，
不会留下半份 TOML；新文件与快照都只有当前用户可读写。

同一文件若被另一个窗口或编辑器改过，保存会被拒绝，并就地给出两条路：**重新读取**
（丢掉我的改动）或**另存为**——**没有"直接覆盖"这个按钮**，因为那会盖掉别人刚写进去的。
保存期间继续输入，新输入仍标为未保存。

它和 `fill.py` 读写的是同一份 `content.toml`，两种界面可以来回换：
命令行填一半，网页里接着改，再回命令行都认，历史快照也共用。
...
回车保留、输入覆盖。覆盖前旧内容会留一份到 `content.toml` 旁边的 `.history/`（同样不进 Git），
写完之后会打印快照的路径。内容一字未改时不会写盘，也不会留快照。
...
- `content_io.py` —— 共享读写层：继承、路径约束、版本冲突检查、历史快照和原子保存。
  文件名怎么收（`acme` → `content.acme.toml`）也只在这里定义一次，网页通过
  `/api/stat` 问它，不自己算。不依赖 WeasyPrint，所以 `fill.py --check` 不需要先装排版引擎。
...
- `webui/` —— 网页表单的静态文件，按"一件事一个文件"分：
  `ui.js`（对话框、提示条、横幅）、`form.js`（照着字段表长表单）、
  `preview.js`（防抖预览、页数与空白、纸张缩放）、`export.js`（生成与下载链接）、
  `files.js`（打开 / 新建 / 保存 / 另存为 / 历史版本 / 草稿）、
  `app.js`（状态、接口、启动、事件绑定，最后加载）。
  表单本身不写在这里：它由 `form.js` 照着 `schema.py` 的字段表长出来。
...
python render.py [--content PATH] [--theme PATH] [--css PATH]
                 [--out-dir DIR] [--name BASENAME] [--if-exists {overwrite,rename,fail}]
```
...
- `--if-exists` 生成物同名时怎么办，默认 `overwrite`（直接覆盖，等于从前的行为）。
  `rename` 自动改成 `X-2`（三种后缀一起避开，不会出现"新 PDF 配旧 PNG"）；
  `fail` 直接报错、不动任何文件。网页默认的是 `fail`——那里多问一句几乎没成本，
  而无声覆盖掉上一次的生成物是不可逆的。
...
5. `git status` 应该是干净的。用过网页版的话，顺手看一眼有没有留下
   ...
   内容目录下的 `.history/` 也一样：同样被挡着，但那是你的真实内容，别哪天手动
   把它加进 Git。
...
默认的四个正文板块（技能 / 经历 / 项目 / 教育）顺序固定、内容不能超过一页。
想调这几样，浏览器里最省事（每个正文板块标题旁有上移 / 下移 / 移除，表单末尾能
新建自定义板块），也可以直接在 TOML 里写三个可选的顶层键。**没写这三个键的文件
和改造前逐字节一致**，行为完全不变。

**`body_order`——正文板块的顺序**

```toml
# 写在任何 [表] 之前（它是根级键）。列进来的才渲染，没列的内建块=已移除。
body_order = ["experiences", "skills", "projects", "education"]
```

列表里去掉某个内建块名，它就不再渲染——但**数据仍留在文件里**，把名字加回来
就恢复了，移除是可逆的。自定义块在列表里写成 `"custom:<key>"`。

**`[[custom_sections]]`——自己定的新板块**

```toml
[[custom_sections]]
key = "awards"          # ASCII 标识符，内部用，不显示
title = "获奖情况"       # 左侧竖脊上的栏目名
identity = "name"       # 认条目用哪个字段（[keep] 按名挑时用它）

  [[custom_sections.fields]]
  key = "name"
  ask = "奖项"
  kind = "text"         # text 单行 / lines 多行每行一条 / list 一行多项用 / 分隔
  required = true       # 第一个字段当标题，必填；其余可留空
...
字段定义随内容文件走（不进 `schema.py`）。自定义块用一套**通用版面**渲染：
第一个 text 字段当标题，`list` 当面包屑，`lines` 当 bullet，其余 text 当正文段落。
没有内建块那种手调版面，换来的是字段完全自定义。

**`max_pages`——放宽页数上限**

```toml
max_pages = 2          # 根级键，1..5，默认 1
```

默认仍是一页。有些经历多的岗位写不进一页，把它调到 2–5 就行——超过设定的上限
才报错。仍然不建议靠缩小字号硬塞（见下一节）。
...
| 网页版存不了，徽标写着"只读" | 打开的是那份示例，或者是一份定制版（`extends`） | 用「另存为…」存成 `content.<公司>.toml`；定制版请手工编辑那十几行 |
| 网页版保存被拒，提示"被改过" | 同一份文件被另一个窗口或编辑器改过 | 横幅上就地两个选择：重新读取（丢掉我的改动）或另存为。没有"强制覆盖" |
| 「另存到…」按钮是灰的，生成对话框里也没有"存到哪" | 浏览器没有 File System Access API（Firefox、Safari 都没有） | 用「下载」；或者换 Chrome / Edge 打开这个页面 |
| 生成时提示"已经有一份 X.pdf" | 同名的生成物还在 `build/` 里 | 对话框里二选一：自动改名，或覆盖。命令行要同样的行为用 `--if-exists rename` |
| 选了"只存到我选的位置"，结果那个文件是 0 字节 | 挑好位置之后写入才失败（磁盘满、U 盘被拔、权限） | 横幅会直接说这事。这条路不留副本，得重新生成一次 |
| 「在文件夹中显示」说打不开 | 这个环境里没有文件管理器可用（无桌面的 Linux） | 按提示复制路径自己找过去；WSL 里应当能调起 Windows 的资源管理器 |
| 想找回上一次保存前的内容 | 每次覆盖前都留了快照 | 顶栏「历史版本…」，挑一版读回表单；它就在内容目录的 `.history/` 里 |
